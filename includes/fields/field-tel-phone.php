...
add_action( 'wpcf7_init', 'wpcf7_add_form_tag_phone' );

function wpcf7_add_form_tag_phone() {
	wpcf7_add_form_tag(
		array( 'phone', 'phone*' ),
		'wpcf7_phone_form_tag_handler', array( 'name-attr' => true ) );
}

function wpcf7_phone_form_tag_handler( $tag ) {
	if ( empty( $tag->name ) ) {
		return '';
	}
...
add_action( 'wpcf7_admin_init', 'wpcf7_add_tag_generator_phone', 16 );

function wpcf7_add_tag_generator_phone() {
	$tag_generator = WPCF7_TagGenerator::get_instance();
	$tag_generator->add( 'phone', __( 'phone', 'contact-form-7' ),
		'wpcf7_tag_generator_phone' );
}

function wpcf7_tag_generator_phone( $contact_form, $args = '' ) {
	$args        = wp_parse_args( $args, array() );
	$description = __( "Generate a form-tag for a single-line telephone number input field. Formatted by USA standart or Custom Format", 'contact-form-7' );
	?>
